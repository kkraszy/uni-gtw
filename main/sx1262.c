...
#define SX_SET_DIO2_AS_RF_SWITCH_CTRL 0x9D
...
static const uint32_t sx1262_channel_freq[2] = {
    (uint32_t)((uint64_t)868000000ULL * (1ULL << 25) / 32000000UL),
    (uint32_t)((uint64_t)868200000ULL * (1ULL << 25) / 32000000UL),
};
...
static esp_err_t sx1262_ops_init(const struct hardware_config_t *hw,
  ...
  {
    uint8_t p = 1;
    sx1262_cmd(SX_SET_DIO2_AS_RF_SWITCH_CTRL, &p, 1);
